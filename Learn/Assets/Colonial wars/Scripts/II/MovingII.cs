...
{   //Скрипт передвижения персонажей и хранения ресурсов у крестьян
    //Крестьянин может переносить только одну еденицу ресурса
    public float Speed; // Скорость юнита
    private int _resoursFood; //Временные переменные для хранения ресурсов для переноски еды
    private int _resoursWood; //Временные переменные для хранения ресурсов для переноски дерева
    private int _resoursStone; //Временные переменные для хранения ресурсов для переноски камня
    public Transform TargetSclad; //Точка склада
    public Transform TargetIstochnic; //Точка источника 
    private Animator _animator;
    private bool _isMau = true; //переменная для поворота
    private bool isCome = true; //Переменная для захода в тригер
    public Transform Collecting; // Место сбора свободных крестьян
    ...
    //У обьекта ресурса должен быть коллайдер с галочкой Тригер
